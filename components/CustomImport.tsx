...
const CustomImport: React.FC<CustomImportProps> = ({ savedSets, onImport, onDeleteSet, onApplySet, onCancel }) => {
  ...
  const handleGenerate = async () => {
    if (!setName.trim() || !bulkText.trim()) {
      alert("Please provide both an archive name and vocabulary text.");
      return;
    }
    ...
    try {
      ...
      // FIXED: Using a valid public model name
      const model = genAI.getGenerativeModel({ model: "gemini-1.5-flash" });
      
      const prompt = `You are an elite IELTS tutor. Extract Band 7.0+ terms from this text: "${bulkText.substring(0, 10000)}".
        Return ONLY a JSON array of objects with: term, vietnameseTranslation, definition, example, bandLevel, collocations (Array of 3).`;
    ...
    <div className="max-w-3xl mx-auto p-8 bg-white border-2 border-[#C5A059]/30 rounded-3xl shadow-2xl">
      ...
      {method === 'MANUAL' ? (
        <div className="space-y-6">
          <input type="text" className="w-full p-4 bg-slate-50 border rounded-xl" placeholder="Set Name" value={setName} onChange={(e) => setSetName(e.target.value)} />
          <textarea className="w-full h-48 p-4 bg-slate-50 border rounded-xl resize-none" placeholder="Paste text here..." value={bulkText} onChange={(e) => setBulkText(e.target.value)} />
        </div>
      ) : (
        <div className="space-y-4 min-h-[200px]">
          {savedSets.length === 0 ? <p className="text-center text-slate-400 py-10">Vault is empty.</p> : savedSets.map(set => (
            <div key={set.id} className="p-4 border rounded-xl flex justify-between items-center">
              ...
              <button onClick={() => onApplySet(set.words)} className="px-4 py-2 bg-[#C5A059] text-white text-xs rounded-lg">Study</button>
            </div>
          ))}
        </div>
      )}

      {isLoading && <p className="text-center text-[#C5A059] font-bold italic my-4 animate-pulse">{loadingStep}</p>}
